...
class FollowerParser:
        ...
        try:
            if soup.find("html").has_attr("lang"):
                ...
                if mcode:
                    ...
                    if follower_tags:
                        self.__write_logs("__parse_case1: " + str(murl))
                        tcount = self.__parse_case1(follower_tags)
                        self.__write_logs("__parse_case1: " + str(murl) + "  " + str(tcount))
                    else:
                        follower_tags = soup.select("ul.user-stats.clearfix")
                        if follower_tags:
                            self.__write_logs("__parse_case2: " + str(murl))
                            tcount = self.__parse_case2(follower_tags)
                            self.__write_logs("__parse_case2: " + str(murl) + "  " + str(tcount))
                        else:
                            follower_tags = soup.select("table.stats.js-mini-profile-stats")
                            if follower_tags:
                                self.__write_logs("__parse_case3: " + str(murl))
                                tcount = self.__parse_case3(follower_tags)
                                self.__write_logs("__parse_case3: " + str(murl) + "  " + str(tcount))
                            else:
                                follower_tags = soup.select("ul.stats.js-mini-profile-stats")
                                if follower_tags:
                                    self.__write_logs("__parse_case4: " + str(murl))
                                    tcount = self.__parse_case4(follower_tags)
                                    self.__write_logs("__parse_case4: " + str(murl) + "  " + str(tcount))
                                else:
                                    follower_tags = soup.select("div.stats")
                                    if follower_tags:
                                        self.__write_logs("__parse_case5: " + str(murl))
                                        tcount = self.__parse_case5(follower_tags)
                                        self.__write_logs("__parse_case5: " + str(murl) + "  " + str(tcount))
                                    else:
                                        follower_tags = soup.select("div#section")
                                        if follower_tags:
                                            self.__write_logs("__parse_case6: " + str(murl))
                                            tcount = self.__parse_case6(follower_tags)
                                            self.__write_logs("__parse_case6: " + str(murl) + "  " + str(tcount))   
                                        else:
                                            ...
                                            if follower_tags:
                                                ...
                                                return
                    self.__logger.debug_log.debug("URIM: {} Converted: {}".format(murl, tcount))
    ...
    def __parse_case1(self, follower_tags):
        for tags in follower_tags:
            self.__logger.debug_log.debug(str(tags))
            fcount_temp = None
    ...
    def __parse_case2(self, follower_tags):
        self.__write_logs(follower_tags[0])
    ...
    def __parse_case3(self, follower_tags):
        ...
        #    self.__write_logs(i)
        self.__write_logs(follower_tags[0].select("a.js-nav")[2])
    ...
    def __parse_case4(self, follower_tags):
        for i in follower_tags[0].select("a.js-nav"):
            self.__write_logs(i)
        self.__write_logs(follower_tags[0].select("a")[2])
